...
export default function LoginRH() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        // --- LÓGICA MESTRE (LAERTE) ---
        if (data.isMaster) {
          localStorage.setItem("role", "MASTER");
          localStorage.setItem("rh_nome", data.nome);
          window.location.href = "/admin"; // Vai para o Painel de Gestão de Empresas
          return;
        }

        // --- LÓGICA RH COMUM ---
        localStorage.setItem("rh_id", data.rh_id);
        localStorage.setItem("rh_nome", data.nome);
        localStorage.setItem("empresa_id", data.empresa_id);
        localStorage.setItem("razao_social", data.razao_social);
        localStorage.setItem("rh_cnpj", cnpj); // Salva para exibir no dashboard
        
        window.location.href = "/rh"; // Vai para o Painel de Ocorrências
      } else {
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 font-sans selection:bg-emerald-100">
      <div className="max-w-md w-full animate-in fade-in zoom-in duration-500 space-y-8">
        ...
        <div className="bg-white rounded-[2.5rem] shadow-2xl shadow-slate-200/50 p-10 border border-slate-100 relative overflow-hidden">
          ...
          <h2 className="text-xl font-bold text-slate-800 mb-8 text-center uppercase tracking-tighter relative z-10">Acesse sua Conta</h2>
          ...
          <form onSubmit={handleLogin} className="space-y-6 relative z-10">
            <div className="space-y-1.5">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Identificação / CNPJ</label>
