...
export default function PainelEmpregado() {
  ...
  useEffect(() => {
    denunciaAtivaRef.current = denunciaAtiva;
  }, [denunciaAtiva]);
  ...
  // Usa ref para evitar o problema de "stale closure" com o setInterval
  ...
  // Para o polling quando o chat fecha
  useEffect(() => {
    if (!denunciaAtiva) {
      if (intervalRef.current) {
        clearInterval(intervalRef.current);
  ...
  const enviarMensagem = async () => {
    ...
    try {
      const res = await fetch("/api/chat", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          protocolo: denunciaAtiva.protocolo,
          remetente: "EMPREGADO",
  ...
  const getStatusVisual = (status: string) => {
    switch (status) {
      case "PENDENTE":
        return (
          <span className="flex items-center gap-1.5 text-[10px] font-black text-amber-600 bg-amber-50 px-3 py-1.5 rounded-full border border-amber-100 uppercase tracking-widest">
      ...
      case "EM_ANALISE":
        return (
          <span className="flex items-center gap-1.5 text-[10px] font-black text-blue-600 bg-blue-50 px-3 py-1.5 rounded-full border border-blue-100 uppercase tracking-widest">
            <Activity className="w-3.5 h-3.5" /> Em Análise
          </span>
        );
      case "RESOLVIDO":
        return (
        ...
        {abaAtiva === "nova" && (
          protocoloGerado ? (
            ...
            <form
              ...
              className="bg-white p-8 md:p-10 rounded-[3rem] shadow-2xl border border-slate-100 space-y-8 animate-in slide-in-from-bottom-6 duration-500"
            >
          ...
          <div className="space-y-6 animate-in slide-in-from-bottom-6 duration-500">
            {loadingHistorico ? (
              ...
              historico.map((item, index) => (
                <div
                  key={index}
                  className="bg-white p-8 rounded-[3rem] shadow-xl border border-slate-100 hover:border-emerald-100 transition-all duration-300"
                >
              ...
              {loadingChat ? (
                ...
                <div className="text-center py-8">
                  <p className="text-xs text-slate-400 font-bold uppercase tracking-widest">
                    Nenhuma mensagem ainda. Aguarde a resposta do RH.
                  </p>
                </div>
              ) : (
                mensagens.map((msg: any) => (
                  <div
                    key={msg.id}
                    className={`flex flex-col ${msg.remetente === 'EMPREGADO' ? 'items-end' : 'items-start'} w-full animate-in fade-in slide-in-from-bottom-2`}
                  >
                    <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1 mx-2">
                      {msg.remetente === 'EMPREGADO' ? 'Você' : 'Equipe de RH'}
                    </span>
                    <div
                      className={`p-5 sm:p-6 rounded-3xl shadow-sm max-w-[90%] sm:max-w-[85%] font-bold leading-relaxed text-sm ${msg.remetente === 'EMPREGADO'
            ...
            {denunciaAtiva.status === "RESOLVIDO" ? (
              <div className="bg-slate-100 p-6 text-center border-t border-slate-200">
                <p className="text-xs font-black text-slate-400 uppercase tracking-widest">
                  Este caso foi encerrado e o chat bloqueado.
                </p>
              </div>
            ) : (
              <div className="bg-white p-4 sm:p-6 border-t border-slate-200 shrink-0">
                <div className="flex items-end gap-3 sm:gap-4">
                  ...
                  <button
                    ...
                    className="bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed text-white p-4 sm:p-5 rounded-2xl sm:rounded-3xl shadow-xl shadow-emerald-600/20 transition-all active:scale-95 h-20 sm:h-24 px-6 sm:px-8 flex flex-col items-center justify-center gap-1 sm:gap-2 group"
                  >
                    {enviando
